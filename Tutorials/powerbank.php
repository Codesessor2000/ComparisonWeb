...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Compare Web:PowerBank</title>
...
if(isset($_POST['submit']))
{
  ...
  //Execute query
    $sql = "SELECT * FROM powerbank_table WHERE Model LIKE '%$searchinput%' OR Model LIKE '%$searchinput1%' ";
  ...
  if($res==true)
  {
    ...
    if($count>0)
    {
     ...
     <?php echo $Brand = $rows['Brand'] ?>
      ...
      <td>
      <?php echo $Power_in_Watts = $rows['Power_in_Watts'] ?>
      </td>
      <td>
      <?php echo $Capacity_in_mAH = $rows['Capacity_in_mAH'] ?>
      </td>
      <td>
     ...
     <?php echo $Warranty = $rows['Warranty'] ?>
      ...
      <td>
     <?php echo $Price_in_Rupees = $rows['Price_in_Rupees'] ?>
      </td>
...
inputBox.onkeyup = (e) => {
    ...
    if (userData) {
        ...
        emptyArray = powerbankSuggestions.filter((data) => {
...
inputBox1.onkeyup = (e) => {
    ...
    if (userData1) {
        ...
        emptyArray1 = powerbankSuggestions.filter((data) => {
            //filtering array value and user characters to lowercase and return only those words which are start with user enetered chars
